...
class MainActivity : AppCompatActivity() {
    ...
    private fun getWeatherInfo(cityName: String) {
        ...
        val url ="https://api.weatherapi.com/v1/forecast.json?key=$apiKey&q=$cityName&days=1&aqi=yes&alerts=yes"
        ...
        val jsonObjectRequest = JsonObjectRequest(Request.Method.GET, url,null, {
            response ->
                ...
                try{
                    ...
                    if(isDay==1) {
                        Picasso.get()
                            .load("https://images.unsplash.com/photo-1491961713439-c6a3e7368eff?ixlib=rb-4.0.3&q=85&fm=jpg&crop=entropy&cs=srgb&dl=renato-andrade-fernandes-wk8rpygTnPQ-unsplash.jpg")
                            .into(binding.backgroundImage)

                    } else {
                        Picasso.get()
                            .load("https://images.unsplash.com/photo-1507400492013-162706c8c05e?ixlib=rb-4.0.3&q=85&fm=jpg&crop=entropy&cs=srgb&dl=timothee-duran-ilfsT5p_qvA-unsplash.jpg")
                            .into(binding.backgroundImage)
                    }
                    ...
                    val precip = response.getJSONObject("current").getString("precip_mm")
                    binding.precipmm.text=resources.getString(R.string.precipmm,precip)
                    ...
                    val pressures = response.getJSONObject("current").getString("pressure_mb")
                    binding.pressure.text=resources.getString(R.string.pressuremb,pressures)
                    val vis = response.getJSONObject("current").getString("vis_km")
                    binding.visibility.text=resources.getString(R.string.visibilitykm,vis)
